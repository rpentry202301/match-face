...
import { Users, useUserSelect } from "@/hooks/store/context/UserSelectContext";
import type { FetchUserModalData } from "@/types/admin/tasks/register/types";

const UserSelectModalForm = ({
  fetchData,
}: {
  fetchData: FetchUserModalData;
}) => {
  ...
  const [checkedValues, setCheckedValue] = useState<Users[]>(userSelect);
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const checkedDepId = formData.department
      ...
      .map((dep) => dep.id);
    const checkedStateId = formData.state
      .filter((data) => data.checked)
      .map((state) => state.id);
    const selectedGroup = fetchData.userGroups.filter(
  ...
  const handleChangeUserList = (e: ChangeEvent<HTMLInputElement>) => {
    if (checkedValues.map((value) => value.name).includes(e.target.value)) {
      setCheckedValue(
        checkedValues.filter((value) => value.name !== e.target.value)
      );
    } else {
      setCheckedValue([
        ...checkedValues,
  ...
  return (
    <Modal buttonText="追加" isOpened={isOpened} open={open} close={close}>
      ...
      <form
        ...
        className="flex flex-col justify-center items-center border-2 w-10/12 px-10 py-5 mt-3 mx-auto"
      >
      ...
      <UserList
        users={users || []}
        checkedValues={checkedValues.map((value) => value.name)}
        onChange={handleChangeUserList}
      />
...
export default UserSelectModalForm;
